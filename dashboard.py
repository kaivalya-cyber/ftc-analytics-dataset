...
with st.sidebar:
    ...
    page = st.radio(
        "NAVIGATE",
        ["🏠 Home", "🔍 Team Explorer", "📅 Event Browser", "🏆 OPR Leaderboard", "📈 ELO Ratings", "🤝 Head-to-Head", "⚡ Upset Analysis", "🎯 Match Predictor"],
...
# ============================================================================
    ...
    st.markdown("""
    <div class="hero-header">
        <div class="hero-title" style="font-size:1.8rem;">🤝 Head-to-Head</div>
        <div class="hero-subtitle">Compare any two teams — see their full match history against each other, win/loss records, and score timelines.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    col_a, col_b = st.columns(2)
    with col_a:
        team_a = st.selectbox("Team A", team_list, key="h2h_a", index=0)
    with col_b:
        team_b = st.selectbox("Team B", team_list, key="h2h_b", index=min(1, len(team_list) - 1))

    if team_a and team_b and team_a != team_b:
        # Find all matches where these teams faced each other (on opposite alliances)
        h2h_matches = matches[
            (
                ((matches["red_team_1"] == team_a) | (matches["red_team_2"] == team_a)) &
                ((matches["blue_team_1"] == team_b) | (matches["blue_team_2"] == team_b))
            ) |
            (
                ((matches["red_team_1"] == team_b) | (matches["red_team_2"] == team_b)) &
                ((matches["blue_team_1"] == team_a) | (matches["blue_team_2"] == team_a))
            )
        ].sort_values(["season", "event_key", "match_number"])
        ...
        if len(h2h_matches) == 0:
            ...
            # Compute H2H stats
            a_is_red = (
                (h2h_matches["red_team_1"] == team_a) | (h2h_matches["red_team_2"] == team_a)
            )
            a_wins = ((a_is_red) & (h2h_matches["winner"] == "red")).sum() + \
                     ((~a_is_red) & (h2h_matches["winner"] == "blue")).sum()
            b_wins = ((a_is_red) & (h2h_matches["winner"] == "blue")).sum() + \
                     ((~a_is_red) & (h2h_matches["winner"] == "red")).sum()
            ties = (h2h_matches["winner"] == "tie").sum()

            # H2H stat cards
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                st.markdown(stat_card_html("⚔️", str(len(h2h_matches)), "Total Matches", "accent-red"), unsafe_allow_html=True)
            with c2:
                st.markdown(stat_card_html("🏆", str(a_wins), f"Team {team_a} Wins", "accent-blue"), unsafe_allow_html=True)
            with c3:
                st.markdown(stat_card_html("🏆", str(b_wins), f"Team {team_b} Wins", "accent-orange"), unsafe_allow_html=True)
            with c4:
                st.markdown(stat_card_html("🤝", str(ties), "Ties", "accent-green"), unsafe_allow_html=True)
            ...
            for _, r in h2h_matches.iterrows():
                ...
                if a_red:
                    a_scores.append(r["red_score"])
                    b_scores.append(r["blue_score"])
                else:
                    a_scores.append(r["blue_score"])
                    b_scores.append(r["red_score"])

            st.markdown("---")
            c1, c2 = st.columns(2)
            with c1:
                c1.metric(f"Team {team_a} Avg Score", f"{np.mean(a_scores):.1f}")
                c1.metric(f"Team {team_b} Avg Score", f"{np.mean(b_scores):.1f}")
            with c2:
                c1, c2, _ = st.columns([1, 1, 1])
                with c1:
                    st.metric("Max Margin", f"{max(abs(np.array(a_scores) - np.array(b_scores))):.0f}")
                with c2:
                    st.metric("Seasons", str(h2h_matches["season"].nunique()))

            # Match history table
            st.markdown("### 📋 Match History")
            h2h_display = h2h_matches[["season", "event_key", "match_number", "red_team_1", "red_team_2", 
                                         "blue_team_1", "blue_team_2", "red_score", "blue_score", "winner", "is_playoff"]].copy()
            h2h_display["Result"] = h2h_display.apply(
                lambda r: f"{int(r['red_score'])}–{int(r['blue_score'])}", axis=1
            )
            st.dataframe(
                h2h_display[["season", "event_key", "match_number", "Result", "winner", "is_playoff"]].rename(
                    columns={"season": "Season", "event_key": "Event", "match_number": "Match #", 
                             "is_playoff": "Playoff", "winner": "Winner"}
                ),
                use_container_width=True,
                hide_index=True,
            )
            ...
            timeline = pd.DataFrame({
                "Match #": range(1, len(h2h_matches) + 1),
                f"Team {team_a}": a_scores,
                f"Team {team_b}": b_scores,
            }).set_index("Match #")
...
# UPSET ANALYSIS
# ============================================================================
elif page == "⚡ Upset Analysis":
    st.markdown("""
    <div class="hero-header">
        <div class="hero-title" style="font-size:1.8rem;">⚡ Upset Analysis</div>
        <div class="hero-subtitle">
            Discover when underdogs win — tracking matches where the predicted favorite (by OPR or ELO) loses.
        </div>
    </div>
    """, unsafe_allow_html=True)

    # Compute upset stats on the fly
    upset_rows = []
    for _, row in matches.iterrows():
        ...
        upset_rows.append({
            "season": row["season"], "is_playoff": row["is_playoff"],
            "opr_upset": opr_upset, "elo_upset": elo_upset,
            "opr_diff": abs(red_opr - blue_opr),
            "red_score": row["red_score"], "blue_score": row["blue_score"],
            "winner": winner, "opr_fav": opr_fav,
            "red_team_1": r1, "red_team_2": r2, "blue_team_1": b1, "blue_team_2": b2,
            "event_key": row["event_key"], "match_number": row["match_number"],
        })
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(stat_card_html("⚡", f"{opr_upsets/total*100:.1f}%", "OPR Upset Rate", "accent-red"), unsafe_allow_html=True)
    with c2:
        st.markdown(stat_card_html("📊", f"{elo_upsets/total*100:.1f}%", "ELO Upset Rate", "accent-blue"), unsafe_allow_html=True)
    with c3:
        st.markdown(stat_card_html("🎯", f"{total:,}", "Matches Analyzed", "accent-orange"), unsafe_allow_html=True)
    with c4:
        st.markdown(stat_card_html("🤖", f"{opr_upsets}", "Total OPR Upsets", "accent-green"), unsafe_allow_html=True)

    # By season chart
    st.markdown("---")
    st.markdown("### 📊 Upset Rate by Season")
    season_upset_stats = upset_df.groupby("season").agg(
        Matches=("opr_upset", "count"),
        OPR_Upset_Rate=("opr_upset", "mean"),
        ELO_Upset_Rate=("elo_upset", "mean"),
    )
    ...
    st.dataframe(
        season_upset_stats.rename(columns={"OPR_Upset_Rate": "OPR Upset %", "ELO_Upset_Rate": "ELO Upset %"}),
        use_container_width=True,
        column_config={
            "OPR Upset %": st.column_config.NumberColumn(format="%.1f%%"),
            "ELO Upset %": st.column_config.NumberColumn(format="%.1f%%"),
        },
    )
    ...
    st.markdown("---")
    st.markdown("### 🔥 Biggest OPR Upsets")
    biggest = upset_df[upset_df["opr_upset"]].nlargest(15, "opr_diff")
    biggest_display = biggest[["season", "event_key", "match_number", "red_team_1", "red_team_2", 
                                 "blue_team_1", "blue_team_2", "red_score", "blue_score", "opr_diff", "winner"]].copy()
    ...
    st.dataframe(
        biggest_display[["season", "event_key", "match_number", "red_team_1", "red_team_2", 
                         "blue_team_1", "blue_team_2", "Score", "opr_diff", "winner"]].rename(
            columns={"season": "Season", "event_key": "Event", "match_number": "Match #", 
                     "red_team_1": "R1", "red_team_2": "R2", "blue_team_1": "B1", "blue_team_2": "B2",
                     "opr_diff": "OPR Diff", "winner": "Winner"}
        ),
        use_container_width=True,
        hide_index=True,
        column_config={
            "OPR Diff": st.column_config.NumberColumn(format="%d"),
        },
    )
